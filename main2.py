...
class HardMazeGame:
    def __init__(self, width=35, height=35):
        ...
        self.map_revealed = False  # New: map reveal state
        self.show_full_solution = False  # New: full solution display state
    ...
    def draw_maze(self):
        ...
        for y in range(self.height):
            for x in range(self.width):
                ...
                is_visible = self.map_revealed or distance <= vision_range

                if is_visible:
                    if self.maze[y][x] == 1:  # Wall
                        color = "#444444" if not self.map_revealed else "#666666"
                        outline = "#666666" if not self.map_revealed else "#888888"
                        self.canvas.create_rectangle(
                            x1, y1, x2, y2, fill=color, outline=outline
                        )
                    else:  # Path
                        color = "#f0f0f0" if not self.map_revealed else "#e8e8e8"
                        outline = "#cccccc" if not self.map_revealed else "#d0d0d0"
                        self.canvas.create_rectangle(
                            x1, y1, x2, y2, fill=color, outline=outline
                        )
                else:
    ...
    def toggle_map_view(self):
        """Toggle between limited vision and full map view"""
        self.map_revealed = not self.map_revealed
        if self.map_revealed:
            messagebox.showinfo(
                "Map Revealed!",
                "🗺️ FULL MAP REVEALED!\n\n"
                "• All walls and paths visible\n"
                "• Red ⚠ squares are deadly traps!\n"
                "• Purple ◉ are teleporters\n"
                "• Gold 🗝 are keys to collect\n"
                "• Red 👹 is the moving enemy\n"
                "• Press M again to return to limited vision",
            )
        else:
            messagebox.showinfo("Map Hidden", "Returned to limited vision mode")

        self.draw_maze()

    def show_solution(self):
        """Show the solution path using selected algorithm with full visual"""
        algorithm = self.solver_var.get()
        start_pos = self.player_pos[:]

        # Temporarily reveal map for solving
        was_revealed = self.map_revealed
        self.map_revealed = True
        self.show_full_solution = True
        ...
        # Get solution path
        path_result = self.find_path(start_pos, target, algorithm)
        ...
        # Redraw maze with full visibility
        self.draw_maze()
        ...
        if path:
            ...
            # Draw solution path with enhanced visuals
            ...
            self.canvas.create_oval(
                ...
                (start_x + 1) * self.cell_size - 2,
                (start_y + 1) * self.cell_size - 2,
                outline="lime",
                width=3,
                tags="solution",
            )
            ...
            self.canvas.create_oval(
                ...
                (end_x + 1) * self.cell_size - 2,
                (end_y + 1) * self.cell_size - 2,
                outline="red",
                width=3,
                tags="solution",
            )

            # Show detailed solution info
            messagebox.showinfo(
                "🔍 SOLUTION FOUND!",
                ...
                f"Press M to toggle map view on/off",
            )
        else:
            messagebox.showwarning(
        ...
        # Restore original map view state if it was hidden
        if not was_revealed:
...
"""
    ...
    def find_path_ucs(self, start, end):
        """Uniform-Cost Search - Like BFS but with priority queue for weighted graphs"""
        import heapq
    ...
    def reset_game(self):
        ...
        self.has_flashlight = False
        self.flashlight_start = None
        self.map_revealed = False  # Reset map view
        self.show_full_solution = False
        self.setup_difficulty_features()
        self.start_time = time.time()
        self.draw_maze()
